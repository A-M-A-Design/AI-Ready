/**
 * WDS MCP Extractor v6.3 — Single Component Extraction
 * 
 * Version conforme au schéma mcp-extraction.schema.json
 * 
 * Usage:
 ...
 * 
 * Output:
 *   JSON conforme au schéma mcp-extraction.schema.json
 *   Vérifier metadata._error pour les échecs.
 * 
 * Changelog v6.3:
 *   - figmaId (string) → figma_ids (array)
 *   - vc → metadata.variant_count
 *   - dv → metadata.default_variant
 *   - meta.nid/pid → metadata.figma_source.nodeId/pageId
 *   - _stats → metadata.stats
 *   - _extracted_at → metadata.extracted_at
 *   - Ajout metadata.extractor_version
 */
...
const EXTRACTOR_VERSION = "mcp_v6.3";
...
try {
  ...
  // ════════════════════════════════════════════════════════════
  // RETURN RESULT — CONFORME AU SCHÉMA mcp-extraction.schema.json
  // ════════════════════════════════════════════════════════════
  ...
  return {
    ...
    metadata: {
      figma_source: {
        nodeId: cs.id,
        pageId: cs.parent?.id || null
      },
      variant_count: cs.children.length,
      default_variant: dv.name,
      extracted_at: new Date().toISOString(),
      extractor_version: EXTRACTOR_VERSION,
      stats: {
